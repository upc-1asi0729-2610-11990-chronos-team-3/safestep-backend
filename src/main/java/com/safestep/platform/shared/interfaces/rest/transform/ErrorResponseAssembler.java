...
public final class ErrorResponseAssembler {
    ...
    /**
     * Maps an ApplicationError to an appropriate HTTP ResponseEntity. Automatically selects the correct HTTP status
     * code based on the error code.
     *
     * @param error
     *            the ApplicationError to map
     *
    ...
    private static String toLocalizedMessageFromApplicationError(ApplicationError error) {
        ...
        return toLocalizedMessageWithFallback(fallbackKey, error.message(), error.details(),
                toEntityNameFromErrorCode(error.code()));
    }
     ...
     *
     * @param errorCode
     *            the error code string (e.g., "PROFILE_NOT_FOUND", "VALIDATION_ERROR")
     *
